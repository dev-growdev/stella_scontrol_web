...
export function IsRateable({
  ...
  const [totalApportionmentsValueState, setTotalApportionmentsValueState] =
    useState(0);
  ...
  const shouldDisableButtonAdd = (
    totalApportionmentsValueState: number,
    totalValue: number
  ) => {
    return (
      totalApportionmentsValueState === totalValue &&
      totalApportionmentsValueState !== 0 &&
      totalValue !== 0
    );
  };
  ...
  useEffect(() => {
    totalApportionmentsValue(
      formatedNumeral(totalApportionmentsValueState).toString()
    );
  ...
  const handleChangeAccountingAccount = (
    event: ChangeEvent<HTMLInputElement>
  ) => {
    if (event.target.outerText) {
      const findAccountingAccount = accountingAccounts.find(
        (acc) => acc.name === event.target.outerText
      );
  ...
  const handleSubmitApportionments = () => {
    ...
    const validateIfValueExceeded =
      parseFloat(valueCostCenter.replace(",", ".")) +
      totalApportionmentsValueState;
    ...
    if (apportionments) {
      ...
      if (isDuplicated) {
        return setError("apportionments", {
          message:
            "Este centro de custo e conta contábil já foram adicionados.",
        });
      }
    }
  ...
  return (
    <div className="flex flex-col">
      <div className="flex items-center">
        <Typography className="mr-10" color="GrayText">
          Rateio
        </Typography>
